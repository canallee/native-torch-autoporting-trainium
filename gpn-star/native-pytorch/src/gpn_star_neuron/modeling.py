...
class GPNStarForMaskedLM(GPNStarPreTrainedModel):
    ...
    # transformers 5.x expects a DICT (a list raises `'list' object has no attribute 'keys'` in
    # post_init/get_expanded_tied_weights_keys). The checkpoint stores `cls.predictions.decoder.weight`;
    # only `decoder.bias` is absent and is tied to `cls.predictions.bias` by the loader. No math change.
    _tied_weights_keys = {}
