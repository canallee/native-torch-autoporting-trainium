...
class EsmConfig(PretrainedConfig):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # NEURON PATCH (compat, used-path config fix): transformers 4.x `PretrainedConfig`
        # populated `is_decoder` / `add_cross_attention` defaults on the base config; transformers 5.x
        # (Beta-3 stack) no longer does, and the (unmodified) modeling code reads `config.is_decoder`
        # in EsmSelfAttention.__init__. Restore the 4.x defaults so the encoder builds. Both are False
        # for this bidirectional DNA encoder — behavior is identical to the original.
